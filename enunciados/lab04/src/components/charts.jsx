    ...
    <ResponsiveContainer>
      <PieChart>
        ...
        <Tooltip
          content={
            <TooltipBox
              valueFmt={(v) => `${formatInt(v)} PRs (${((v / total) * 100).toFixed(1)}%)`}
            />
          }
        />
        <Legend
...
export function MedianByLanguage({ byLanguage, metricKey }) {
...
export function MetricHistogram({ bins, color = '#6366f1', suffix = ' PRs' }) {
  return (
    <ResponsiveContainer>
      <BarChart data={bins} margin={{ left: 4, right: 8 }}>
        <CartesianGrid vertical={false} strokeDasharray="3 3" stroke="#e2e8f0" />
...
/** Barras agrupadas comparando os dois grupos do estudo numa métrica. */
export function GroupComparison({ byGroup, metricKey, statistic = 'median' }) {
